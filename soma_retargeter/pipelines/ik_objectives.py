...
def _smooth_joint_filter_residuals(
    ...
    reference_q: wp.array2d(dtype=wp.float32),       # (n_batch, n_coords)
    ...
    reference_coord_masks: wp.array1d(dtype=wp.float32), # (n_coords)
    ...
    if mask > 0.0:
        ...
        reference_mask = reference_coord_masks[coord_idx]
        c = wp.where(reference_mask > 0.0, reference_q[problem, coord_idx], midpoint)

        lower_error = lower - c
        upper_error = upper - c
        lower_clearance = c - lower
        upper_clearance = upper - c
        padding = wp.float32(1.02)
        safe_lower = wp.where(lower_clearance > padding, lower_error + padding, lower_error)
        safe_upper = wp.where(upper_clearance > padding, upper_error - padding, upper_error)

        q = joint_q[problem, coord_idx]
        error = (q - c)

        if safe_lower < error and error <= safe_upper:
            smoother = wp.float32(0.0)
        else:
            diff = wp.where(error <= safe_lower, safe_lower - error, error - safe_upper)
            smoother = wp.float32(1.0) - wp.exp(-wp.pow(diff, wp.float32(6.5)))
...
class IKSmoothJointFilter(ik.IKObjective):
    ...
    def __init__(
        ...
        self.reference_coord_masks = None
        self.reference_coord_masks_np = None
        if reference_coord_masks is not None:
            if isinstance(reference_coord_masks, np.ndarray):
                self.reference_coord_masks_np = reference_coord_masks.astype(np.float32)
            elif isinstance(reference_coord_masks, wp.array):
                self.reference_coord_masks = reference_coord_masks
    ...
    def init_buffers(self, model, jacobian_mode):
        ...
        if (
            self.reference_coord_masks_np is not None
            and len(self.reference_coord_masks_np) == model.joint_coord_count
        ):
            self.reference_coord_masks = wp.array(
                self.reference_coord_masks_np,
                dtype=wp.float32,
                device=self.device,
            )
        if self.reference_coord_masks is None:
            self.reference_coord_masks = wp.zeros(
                shape=model.joint_coord_count,
                dtype=wp.float32,
                device=self.device,
            )
        self.reference_q = wp.zeros(
    ...
    def set_reference(self, reference_q_np):
        if self.reference_q is None:
            return
        reference = np.asarray(reference_q_np, dtype=np.float32)
        if reference.ndim == 1:
            reference = reference[None, :]
        wp.copy(self.reference_q, wp.array(reference, dtype=wp.float32, device=self.device))
...
def _temporal_reference_residuals(
    ...
    problem_masks: wp.array1d(dtype=wp.float32),
    ...
    ) * weight[0] * mask * problem_masks[problem]
...
def _temporal_reference_jac_analytic(
    ...
    jacobian[problem, start_idx + dof_idx, dof_idx] = (
        weight[0] * coord_masks[coord_idx] * problem_masks[problem]
    )
...
class IKTemporalJointReference(ik.IKObjective):
    ...
    def __init__(self, joint_coord_count: int, joint_dof_count: int | None = None, weight=0.0, coord_masks=None):
        super().__init__()
        self.n_coords = int(joint_coord_count)
        self.n_dofs = int(joint_dof_count) if joint_dof_count is not None else max(0, self.n_coords - 1)
        self._weight = wp.array([weight], dtype=wp.float32)
        self.target_q = None
        self.dof_to_coord = None
        self.coord_masks = None
        self.problem_masks = None
        self.coord_masks_np = None
        if coord_masks is not None:
            if isinstance(coord_masks, np.ndarray):
                self.coord_masks_np = coord_masks.astype(np.float32)
    ...
    def init_buffers(self, model, jacobian_mode):
        self._require_batch_layout()
        self.n_dofs = int(model.joint_dof_count)
        if self.coord_masks_np is not None and len(self.coord_masks_np) == model.joint_coord_count:
            self.coord_masks = wp.array(self.coord_masks_np, dtype=wp.float32, device=self.device)
        if self.coord_masks is None:
            self.coord_masks = wp.ones(shape=model.joint_coord_count, dtype=wp.float32, device=self.device)
        ...
        for joint_idx in range(model.joint_count):
            dof0 = int(qd_start_np[joint_idx])
            coord0 = int(q_start_np[joint_idx])
            lin, ang = joint_dof_dim_np[joint_idx]
            for offset in range(int(lin + ang)):
                dof_idx = dof0 + offset
                if 0 <= dof_idx < self.n_dofs:
                    dof_to_coord_np[dof_idx] = coord0 + offset
        self.dof_to_coord = wp.array(dof_to_coord_np, dtype=wp.int32, device=self.device)
        ...
        self.problem_masks = wp.ones(shape=self.n_batch, dtype=wp.float32, device=self.device)
    ...
    def set_problem_masks(self, masks_np):
        if self.problem_masks is None:
            return
        masks = np.asarray(masks_np, dtype=np.float32).reshape(-1)
        if masks.shape[0] != self.n_batch:
            raise ValueError(
                f"Expected {self.n_batch} temporal reference problem masks, got {masks.shape[0]}"
            )
        wp.copy(self.problem_masks, wp.array(masks, dtype=wp.float32, device=self.device))
...
class IKPointSegmentDistanceBarrier(ik.IKObjective):
    ...
    def __init__(
        ...
        self.n_problems = int(n_problems)
        clearance_value = np.asarray(clearance_m, dtype=np.float32)
        if clearance_value.ndim == 0:
            clearance = np.full((self.n_pairs,), float(clearance_value), dtype=np.float32)
        else:
            clearance = clearance_value.reshape(-1).astype(np.float32)
            if clearance.shape[0] != self.n_pairs:
                raise ValueError(
                    f"clearance_m must be a scalar or have one value per pair: "
                    f"got {clearance.shape[0]} values for {self.n_pairs} pairs"
                )
